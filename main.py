...
from model import PointNet, DGCNN, GBNet, DCTN
import numpy as np
...
def train(args, io):
    ...
    if args.model == 'pointnet':
        if args.dataset == 'modelnet40':
            model = PointNet(num_class=40).to(device)
        elif args.dataset == 'ScanObjectNN':
            model = PointNet(num_class=15).to(device)
        else:
            raise Exception("Dataset Not supported")
        ...
        if args.dataset == 'modelnet40':
            ...
            raise Exception("Dataset Not supported")
    elif args.model == 'dctn':
        if args.dataset == 'modelnet40':
            model = DCTN(num_class=40).to(device)
        elif args.dataset == 'ScanObjectNN':
            model = DCTN(num_class=15).to(device)
        else:
            raise Exception("Dataset Not supported")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='gbnet', metavar='N',
                        choices=['pointnet', 'dgcnn', 'gbnet', 'dctn'],
                        help='Model to use, [pointnet, dgcnn, gbnet, dctn]')
