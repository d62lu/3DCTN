...
class DCTN(nn.Module):
    def __init__(self,num_class,normal_channel=True):
        ...
        # self.global_sa3 = Global_Transformer(avepooling=True, batchnorm=True, attn_drop_value=0, feed_drop_value=0, npoint=512, in_channel=320, out_channels=320, layers=1, num_heads=10, head_dim=32)
    ...
    def forward(self, xyz):
        ...
        if self.normal_channel:
            # norm = xyz[:, 3:, :]
            xyz = xyz[:, :3, :]
            norm = xyz[:, :3, :] # scanobjectnn has no normal vector, so i use the xyz as the input feature
        else:
            norm = None
